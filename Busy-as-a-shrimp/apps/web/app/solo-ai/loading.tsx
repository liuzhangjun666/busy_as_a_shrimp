export default function SoloAiLoading() {
  return (
    <div className="px-4 py-6 md:px-6">
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <div className="h-6 w-36 animate-pulse rounded-full bg-slate-100" />
        <div className="mt-4 h-8 w-48 animate-pulse rounded-xl bg-slate-200/80" />
        ...
        <div className="mt-8 space-y-3">
          {Array.from({ length: 5 }).map((_, index) => (
            ...
            >
              ...
              <div className="mt-4 h-8 w-52 animate-pulse rounded-xl bg-emerald-100" />
            </div>
          ))}
        </div>
      </div>
